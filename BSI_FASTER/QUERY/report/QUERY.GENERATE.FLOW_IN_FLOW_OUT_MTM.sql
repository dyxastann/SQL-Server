...
DECLARE @SQL_QUERY varchar(max), @TARGET DATE = '2023-06-01', @POINTER INT = 1;

WHILE @POINTER <= 1
BEGIN
	DELETE FROM [report].[FLOW_IN_FLOW_OUT_MTM] WHERE TGL_CURRENT = EOMONTH(@TARGET) AND FLAG_DATA = 'KOL 2';

	SET @SQL_QUERY = CONCAT(
		'SELECT
			''KOL 2'' [FLAG_DATA]
			,[TGL_BEFORE].[TGL_DATA] [TGL_BEFORE]
			,[TGL_CURRENT].[TGL_DATA] [TGL_CURRENT]
			,[TGL_CURRENT].[PERIODE]
			,[TGL_CURRENT].[TGL]
		...
		FROM [series].[RCG.LOAN_DAILY.', EOMONTH(@TARGET), ' (FINAL)] [TARGET]
		FULL OUTER JOIN [series].[RCG.LOAN_DAILY.', EOMONTH(DATEADD(MONTH, -1, @TARGET)), ' (FINAL)] [SOURCE]
		...
		OUTER APPLY (
			...
			FROM [series].[RCG.LOAN_DAILY.', EOMONTH(@TARGET), ' (FINAL)]
		) [TGL_CURRENT]
		OUTER APPLY (
			SELECT
				MAX([TGL_DATA]) [TGL_DATA]
				,LEFT(MAX([TGL_DATA]), 7) [PERIODE]
				,RIGHT(MAX([TGL_DATA]), 2) [TGL]
			FROM [series].[RCG.LOAN_DAILY.', EOMONTH(DATEADD(MONTH, -1, @TARGET)), ' (FINAL)]
		) [TGL_BEFORE]
		WHERE
			(
	...
	INSERT INTO [report].[FLOW_IN_FLOW_OUT_MTM] EXEC(@SQL_QUERY);

	DELETE FROM [report].[FLOW_IN_FLOW_OUT_MTM] WHERE TGL_CURRENT = EOMONTH(@TARGET) AND FLAG_DATA = 'NPF';
	...
	PRINT CONCAT(LEFT(@TARGET, 10), CASE WHEN @TARGET = EOMONTH(@TARGET) THEN ' (FINAL)' END);
	SET @TARGET = DATEADD(MONTH, 1, @TARGET);
END