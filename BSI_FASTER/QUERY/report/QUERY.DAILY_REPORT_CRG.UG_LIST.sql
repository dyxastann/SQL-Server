...
WHILE @POINTER <= 1
BEGIN
	DELETE FROM [compile].[LIST_UG.DAILY.COMPILE (CRG)] WHERE [TGL_REPORT] = @tgl_data;
	SET @TARGET_BEFORE = (
		SELECT TOP 1
	...
	SET @SQL_QUERY = CONCAT(
		'SELECT
			...
			,CASE
				WHEN (
					LEFT(COALESCE([BEF].[KOLCIF], [D_1].[KOL_SUPERCIF], ''1''), 1) IN (''2'')
					AND [CUR].[KOLCIF] < COALESCE([BEF].[KOLCIF], [D_1].[KOL_SUPERCIF], ''1'')
				) THEN CONCAT(''Upgrade Kol 2'', CASE WHEN [CUR].[KOLCIF] IN (''2A'', ''2B'') THEN '' Up Bucket'' END,'' by SuperCIF'')
				WHEN (
					LEFT(COALESCE([BEF].[KOLCIF], [D_1].[KOL_SUPERCIF], ''1''), 1) IN (''3'', ''4'', ''5'')
					AND LEFT([CUR].[KOLCIF], 1) IN (''1'', ''2'')
				) THEN ''Upgrade NPF by SuperCIF''
				WHEN (
					LEFT(COALESCE([BEF].[KOLCIF_NonSuper], ''1''), 1) IN (''2'')
					AND [CUR].[KOLCIF_NonSuper] < COALESCE([BEF].[KOLCIF_NonSuper], ''1'')
				) THEN CONCAT(''Upgrade Kol 2'', CASE WHEN [CUR].[KOLCIF_NonSuper] IN (''2A'', ''2B'') THEN '' Up Bucket'' END,'' by CIF'')
				WHEN (
					LEFT(COALESCE([BEF].[KOLCIF_NonSuper], ''1''), 1) IN (''3'', ''4'', ''5'')
					AND LEFT([CUR].[KOLCIF_NonSuper], 1) IN (''1'', ''2'')
				) THEN ''Upgrade NPF by CIF''
				WHEN (
					LEFT(COALESCE([BEF].[KOLLOAN], ''1''), 1) IN (''2'')
					AND [CUR].[KOLLOAN] < COALESCE([BEF].[KOLLOAN], ''1'')
				) THEN CONCAT(''Upgrade Kol 2'', CASE WHEN [CUR].[KOLLOAN] IN (''2A'', ''2B'') THEN '' Up Bucket'' END,'' by Loan'')
				WHEN (
					LEFT(COALESCE([BEF].[KOLLOAN], ''1''), 1) IN (''3'', ''4'', ''5'')
					AND LEFT([CUR].[KOLLOAN], 1) IN (''1'', ''2'')
				) THEN ''Upgrade NPF by Loan''
			END [FLAG_DATA]
			,NULL [PENYEBAB]
	...
	INSERT INTO [compile].[LIST_UG.DAILY.COMPILE (CRG)] EXEC(@SQL_QUERY);
