...
SET @SQL_QUERY = CONCAT(
	...
	FROM ', @TARGET, '
	WHERE [SUPERCIF] IN (SELECT [SUPERCIF] FROM ', @TARGET, ' WHERE [DIVISI] IN (''KONSUMER'', ''PAWNING'',''HASANAH CARD'') GROUP BY [SUPERCIF]);'
);

BEGIN TRY
	INSERT INTO [request].[FEEDING_DATA.KONSUMER_ALL] (
		[TGL_DATA],
		[NOLOAN],
		[NOMORCIF],
		[SUPERCIF],
...
UPDATE [request].[FEEDING_DATA.KONSUMER_ALL]
SET
...
SET @SQL_QUERY = CONCAT(
	...
	FROM [request].[FEEDING_DATA.KONSUMER_ALL] [TARGET]
